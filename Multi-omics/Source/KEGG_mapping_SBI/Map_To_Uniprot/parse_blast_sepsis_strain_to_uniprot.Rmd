...
taxonomy_table_first <- read_tsv(pipe(paste("awk 'BEGIN {is_include=0;}", 
...
taxonomy_table_second <- read_tsv(pipe(paste("awk 'BEGIN {is_include=0;}", 
                                     " /^\\# Prokaryotes/ {is_include=1;}",
                                     " /^\\## Archaea/ {is_include=0;}", 
                                     "{if(is_include==1){print;}}", "'  < ", 
                                     kegg_taxonomy_file," | grep '^#'")), 
                           col_names = "organism_id")  %>%
                         mutate( organism_id = str_replace_all( organism_id, "^\\#+\\s", "") %>%
                                                str_to_lower())

taxonomy_table <- taxonomy_table_first %>%
                  bind_rows(taxonomy_table_second)
...
rm( "organism_id_vs_pathway_id_table", 
      "taxonomy_table", "taxonomy_table_first", "taxonomy_table_second")
